...
//function SE_Error( const AValue: QWord ): AplString;
//begin
//   case AValue of
//      ERROR_FILE_NOT_FOUND    : Result := 'The specified file was not found.';
//      ERROR_PATH_NOT_FOUND    : Result := 'The specified path was not found.';
//      ERROR_DDE_FAIL          : Result := 'The Dynamic Data Exchange (DDE) transaction failed.';
//      ERROR_NO_ASSOCIATION    : Result := 'There is no application associated with the specified file name extension.';
...
//      else                      Result := 'Unknown error code ('+AplString( IntToStr(AValue) )+')';
//   end;
//end;
...
begin
  ...
  if not Result then RaiseLastOSError; // ShowMessage( SE_Error( exInfo.hInstApp ) {%H-});
end;
